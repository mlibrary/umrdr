...
describe GlobusJob do
  ...
  describe "GlobusJob#copy_complete?" do
    context "directory exists in download dir" do
      before do
        dir = "#{globus_dir}/download/DeepBlueData_id321"
  ...
  describe "GlobusJob#files_prepping?" do
    context "directory exists in prep dir" do
      before do
        dir = "#{globus_dir}/prep/DeepBlueData_id321"
        allow( Dir ).to receive( :exists? ).with( dir ).and_return( true )
      end
  ...
  describe "#globus_error_file" do
    ...
    it "returns the error file name." do
      expect( job.send( :globus_error_file ) ).to eq( "#{globus_dir}/prep/.test.error.DeepBlueData_id321" )
    end
  end
  ...
  context "#globus_error_file_exists?" do
    let( :job ) { j = described_class.new; j.perform( "id321" ); j }
    let( :error_file ) { "#{globus_dir}/prep/.test.error.DeepBlueData_id321" }
  ...
  describe "#globus_job_perform" do
    ...
    let( :lock_file ) { "#{globus_dir}/prep/.test.lock.DeepBlueData_id321" }
  ...
  describe "#globus_lock_file" do
    ...
    it "returns the lock file name." do
      expect( job.send( :globus_lock_file, "id321" ) ).to eq( "#{globus_dir}/prep/.test.lock.DeepBlueData_id321" )
    end
  end
  ...
  context "#target_dir_name" do
    let( :job ) { described_class.new }
    let( :dir ) { "aDir/aSubdir" }
    context "don't create dir." do
      it "returns a target base name." do
        expect( job.send( :target_dir_name, "aDir", "aSubdir" ) ).to eq( dir )
      end
    end
    context "create dir if it doesn't exist." do
      ...
      it "returns a target base name and creates the dir." do
        expect( job.send( :target_dir_name, "aDir", "aSubdir", mkdir: true ) ).to eq( dir )
      end
    end
    context "don't create dir if it exists." do
      ...
      it "returns a target base name and doesn't create the dir." do
        expect( job.send( :target_dir_name, "aDir", "aSubdir", mkdir: false ) ).to eq( dir )
      end
    end
  end
  ...
  describe "#target_download_dir" do
    let( :job ) { described_class.new }
    let( :dir ) { "#{globus_dir}/download/DeepBlueData_id321" }
    it "returns target dowload dir name." do
      expect( job.send( :target_download_dir, "id321" ) ).to eq( dir )
    end
  end
  ...
  describe "#target_file_name" do
    let( :job ) { described_class.new }
    it "returns a target base name." do
      expect( job.send( :target_file_name, "aDir", "aFile" ) ).to eq( "aDir/aFile" )
    end
  end

  describe "#target_file_name_env" do
    let( :job ) { described_class.new }
    let( :file ) { "aDir/.test.atype.basename" }
    it "returns a target base name." do
      expect( job.send( :target_file_name_env, "aDir", "atype", "basename" ) ).to eq( file )
    end
  end

  context "#target_prep_dir" do
    let( :job ) { described_class.new }
    let( :dir ) { "#{globus_dir}/prep/test_DeepBlueData_id321" }
  ...
  context "#target_prep_dir_tmp" do
    let( :job ) { described_class.new }
    let( :dir ) { "#{globus_dir}/prep/test_DeepBlueData_id321_tmp" }
    let( :prefix ) { "#{Rails.env}_" }
