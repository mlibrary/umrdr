  ...
  class WorksReport
    ...
    def self.print_work_line( out, work: nil, work_size: 0, header: false )
      if header
        ...
        out << ',' << work_size.to_s
        out << ',' << "#{human_readable( work_size )}"
        out << ',' << '"' << work.subject.join( '; ' ) << '"'
        out << ',' << '"' << work.creator.join( '; ' ) << '"'
        out << ',' << '"' << "#{work.thumbnail_id.nil? ? '' : work.thumbnail_id}" << '"'
        out << ',' << '"' << "#{work.doi.nil? ? '' : work.doi}" << '"'
      end
      out << "\n"
      out
    end
    ...
    def self.print_file_set_line( out, work_id: nil, file_set: nil, file_size: 0, file_ext: '', header: false )
      if header
        ...
        out << ',' << file_size.to_s
        out << ',' << "#{human_readable( file_size )}"
        out << ',' << file_ext
        out << ',' << '"' << file_set.label << '"'
        out << ',' << '"' << "#{file_set.thumbnail_id.nil? ? '' : file_set.thumbnail_id}" << '"'
      end
      out << "\n"
      out
    end
    ...
    def self.top_ten( hash )
      ...
      hash.each_pair do |key,value|
        if 10 > top.size
          top << [key,value]
          top.sort_by! { |keyValue| 0 - keyValue[1] }
        else
          keyValueToInsert = [key,value]
          top.map! do |keyValue|
            if keyValueToInsert[1] > keyValue[1]
              oldKeyValue = keyValue
              keyValue = keyValueToInsert
              keyValueToInsert = oldKeyValue
            end
            keyValue
          end
        end
    ...
    def self.run
      ...
      all_works.each do |work|
        unless work.nil?
          ...
          work.file_set_ids.each do |fid|
            ...
            print_file_set_line( out_file_sets \
                               , work_id: work.id \
                               , file_set: fs \
                               , file_size: size \
                               , file_ext: ext \
      ...
      count = 0
      authors.each_pair { |key,value| count += 1 if value > 1 }
      works_report << "Repeat authors: #{count}\n"
      works_report << "Unique depositors: #{depositors.size}\n"
      count = 0
      depositors.each_pair { |key,value| count += 1 if value > 1 }
