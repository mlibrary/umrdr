...
module Umrdr
  ...
  class UpdateFileDerivatives
    ...
    def initialize( works, restart_with_work_id: nil )
      ...
      @skip_total_size_for_work = Hash.new
      @tracking = Hash.new { |h,key| h[key] = [] }
    end
    
    def add_to_fids_to_characterize( fid )
      unless @fids_to_characterize_set[fid]
    ...
    def characterize_file_set( fid )
      ...
      if @verbose_file_size
        pacify "{#{human_readable(original_size( af ))}}"
      end
    ...
    def do_characterize( work )
      #return unless @characterize
      if 0 == @fids_to_characterize_list.size && @do_characterize_work
        @fids_to_characterize_list = work.file_set_ids
      end
      return if 0 == @fids_to_characterize_list.size
    ...
    def run
      ...
      begin
        ...
        @works.map do |w|
          work_count += 1
          if 0 == work_count % 10
            puts "(#{work_count}/#{@works.size})"
          end
          if !restarted && !w.nil?
            if @restart_with_work_id == w.id
              restarted = true
            end
          end
          if w.nil?
            ...
            @tracking[:zero_total_file_size] = (@tracking[:zero_total_file_size] << wid) if 0 == w.total_file_size
            @do_characterize_work = w.total_file_size.nil? || 0 == w.total_file_size
            subtotal = 0
    ...
    def test_mime_type( wid, af )
       puts "#{value_or_nil('af.mime_type',af.mime_type)}" if @verbose
