...
namespace :deepblue do
  ...
  task :yaml_populate_from_work, %i[ id options ] => :environment do |_task, args|
    ...
    task = Umrdr::YamlPopulateFromWork.new( id: args[:id], options: args[:options] )
    task.run
  end
  ...
  task :yaml_populate_from_multiple_works, %i[ ids options ] => :environment do |_task, args|
    args.with_defaults( options: '{}' )
    task = Umrdr::YamlPopulateFromMultipleWorks.new( ids: args[:ids], options: args[:options] )
    task.run
  end
  ...
  task :yaml_populate_from_all_works, %i[ options ] => :environment do |_task, args|
    args.with_defaults( options: '{}' )
    task = Umrdr::YamlPopulateFromAllWorks.new( options: args[:options] )
    task.run
  end
...
module Umrdr
  ...
  class YamlPopulateFromAllWorks < Umrdr::YamlPopulate
