...
task :works_and_files_missing_edit_users => :environment do
  WorksAndFilesMissingEditUsers.new().run
end
...
class WorksAndFilesMissingEditUsers < MissingSolrdocs
  ...
  def record_file_set_edit_users( uri, id, fs, depositor, edit_users )
    if edit_users.nil?
  ...
  def record_work_edit_users( uri, id, depositor, edit_users )
    if edit_users.nil?
