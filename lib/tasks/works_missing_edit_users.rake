...
class WorksMissingEditUsers
  ...
  def initialize
    ...
    @work_to_files_missing_edit_users_map = Hash.new
  ...
  def run
    ...
    if @report
      ...
      if @report_files_missing_edit_users
  ...
  def record_to_files_missing_depositor_edit_user( wid, fid )
    @work_to_files_missing_edit_users_map[wid] = [] unless @work_to_files_missing_edit_users_map.has_key? wid
