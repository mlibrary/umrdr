...
class WorksWithMismatchingSolrDocs
  ...
  def initialize
    ...
    @files_mismatching_solr_docs = []
    @to_solr_failed = []
    @work_to_files_mismatching_solr_docs_map = Hash.new
  ...
  def run
    ...
    if @report
      ...
      if @report_files_mismatching_solr_docs
  ...
  def check_work_solr_docs( w )
    ...
    if @test_work_files_for_mismatching_solr_docs
  ...
  def compare_hashes( hash1, hash2, keys_processed )
    ...
    hash1.each do |key,value|
      if keys_processed.has_key? key
        # skip processed key
      elsif @keys_to_skip.has_key? key
        puts "skipping #{key}" if @verbose
      elsif hash2.has_key? key
        value2 = hash2[key]
        if value2 == value
        ...
        puts "skipping key #{key} : '#{value}'" if @verbose
        #puts "skipping key #{key} with nil value" if @verbose
      end
  ...
  def record_collection_solr_docs( w )
    if compare_solr_for_mismatch w
      @collections_mismatching_solr_docs << id
    end
  end

  def record_file_set_solr_docs( w, fs )
    if compare_solr_for_mismatch fs
  ...
  def record_to_files_mismatching_solr_docs( wid, fid )
    @work_to_files_mismatching_solr_docs_map[wid] = [] unless @work_to_files_mismatching_solr_docs_map.has_key? wid
    @work_to_files_mismatching_solr_docs_map[wid] << fid
  end

  def record_work_solr_docs( w )
    if compare_solr_for_mismatch w
      @works_mismatching_solr_docs << w.id
    end
  end
