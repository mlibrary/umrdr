module Hyrax 
  module DepositorsControllerBehavior
    ...
    def destroy
      ...
      send_proxy_depositor_remove_messages(grantor, ::User.from_url_component(params[:id]))
      head :ok
    end
    ...
    private
      ...
      def send_proxy_depositor_remove_messages(grantor, grantee)
        message_to_grantee = "#{grantor.name} has removed you as a proxy depositor"
        message_to_grantor = "You have removed #{grantee.name} as a proxy depositor"
        ::User.batch_user.send_message(grantor, message_to_grantor, "Proxy Depositor Removed")
        ::User.batch_user.send_message(grantee, message_to_grantee, "Proxy Depositor Removed")
      end
