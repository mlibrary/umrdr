...
class Hyrax::GenericWorksController < ApplicationController
  ...
  def download
    ...
    folder = target_dir_name_id( tmp_dir, curation_concern.id )
    zipfile_name = target_file_name_id_ext( folder, curation_concern.id, ".zip" )
  ...
  def target_dir_name_id( base_dir, id )
    target_file_name_id_ext( base_dir, id )
  end

  def target_file_name_id_ext( dir, id, ext = '' )
    folder = dir
    folder = folder + '/' unless folder.end_with? '/'
    folder = folder + "#{Umrdr::Application.config.base_file_name}#{id}#{ext}"
  end

  # def target_dir_name( dir, subdir )
  #   folder = dir
  #   folder = folder + '/' unless ( folder.end_with?( '/' ) || subdir.start_with?( '/' ) )
  #   folder = folder + subdir
  # end

  def target_file_name_ext( dir, filename, ext = '' )
    folder = dir
