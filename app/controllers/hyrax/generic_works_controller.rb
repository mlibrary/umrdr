...
class Hyrax::GenericWorksController < ApplicationController
  ...
  def mark_as_set_to_public!
    ...
    if id and action_name == 'update' and GenericWork.find(id).private?
      visibility_changing_to_public = true
    end
  end

  def visibility_changing_to_public
    @visibility_changed_to_public ||= false
  end

  def visibility_changing_to_public=(bool)
    @visibility_changed_to_public = bool
  end
  ...
  def notify_rdr_on_update_to_public
    return unless visibility_changing_to_public
    location   = main_app.hyrax_generic_work_url(curation_concern.id)
    depositor  = curation_concern.depositor
    title      = curation_concern.title.join("','")
    creator    = curation_concern.creator.join("','")
