...
module Umrdr
  ...
  class YamlPopulateService
    ...
    def yaml_body_collections( out, indent:, curation_concern: )
      ...
      if source == MetadataHelper2::SOURCE_DBDv2
    ...
    def yaml_body_files( out, indent_base:, indent:, curation_concern:, target_dirname: )
      ...
      curation_concern.file_sets.each do |file_set|
        ...
        log_provenance_migrate( curation_concern: file_set, parent: curation_concern ) if MetadataHelper2::MODE_MIGRATE == mode
    ...
    def yaml_filename_collection( pathname_dir:, collection:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_filename( pathname_dir: pathname_dir, id: collection.id, prefix: MetadataHelper2::PREFIX_COLLECTION, task: task )
    end

    def yaml_filename_users( pathname_dir:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_filename( pathname_dir: pathname_dir, id: '', prefix: MetadataHelper2::PREFIX_USERS, task: task )
    end

    def yaml_filename_work( pathname_dir:, work:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_filename( pathname_dir: pathname_dir, id: work.id, prefix: MetadataHelper2::PREFIX_WORK, task: task )
    end
    ...
    def yaml_header_users( out, indent:, header_type: MetadataHelper2::HEADER_TYPE_USERS )
      yaml_line( out, indent, ':ingester:', '' )
      yaml_line( out, indent, ':source:', source )
      yaml_line( out, indent, ':export_timestamp:', DateTime.now.to_s )
      yaml_line( out, indent, ':mode:', mode )
    ...
    def yaml_item_collection( out, indent, curation_concern, name: )
      return if MetadataHelper2::ATTRIBUTE_NAMES_IGNORE.include? name
      label = ":#{name}:"
      value = curation_concern[name]
      return if value.blank? && !MetadataHelper2::ATTRIBUTE_NAMES_ALWAYS_INCLUDE_CC.include?( name )
    ...
    def yaml_item_file_set( out, indent, file_set, name: )
      ...
      label = ":#{name}:"
      value = file_set[name]
      return if value.blank? && !MetadataHelper2::ATTRIBUTE_NAMES_ALWAYS_INCLUDE_FILE_SET.include?( name )
      yaml_item( out, indent, label, value, escape: true )
    end

    def yaml_item_prior_identifier( out, indent, curation_concern: )
      if source == MetadataHelper2::SOURCE_DBDv1
    ...
    def yaml_item_user( out, indent, user, name: )
      return if MetadataHelper2::ATTRIBUTE_NAMES_USER_IGNORE.include? name
      label = ":#{name}:"
      value = user[name]
      return if value.blank? && !MetadataHelper2::ATTRIBUTE_NAMES_ALWAYS_INCLUDE_USER.include?( name )
      yaml_item( out, indent, label, value, escape: true )
    end
    ...
    def yaml_populate_collection( collection:,
      ...
      if out.nil?
        ...
        log_provenance_migrate( curation_concern: collection ) if MetadataHelper2::MODE_MIGRATE == mode
        indent_base = " " * 2
        ...
        yaml_header( out,
                     ...
                     header_type: MetadataHelper2::HEADER_TYPE_COLLECTIONS )
        ...
        indent = indent_base * 2
        yaml_line( out, indent, MetadataHelper2::HEADER_TYPE_WORKS )
        ...
        collection.member_objects.each do |work|
          ...
          log_provenance_migrate( curation_concern: work, parent: collection ) if MetadataHelper2::MODE_MIGRATE == mode
    ...
    def yaml_populate_users( dir: MetadataHelper2::DEFAULT_BASE_DIR, out: nil, target_filename: nil )
      target_file = nil
      dir = Pathname.new dir unless dir.is_a? Pathname
    ...
    def yaml_populate_work( curation_concern:,
      ...
      if out.nil?
        ...
        yaml_header( out,
                     ...
                     header_type: MetadataHelper2::HEADER_TYPE_WORKS )
        indent = indent_base * 2
    ...
    def yaml_targetdir_collection( pathname_dir:, collection:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_targetdir( pathname_dir: pathname_dir, id: collection.id, prefix: MetadataHelper2::PREFIX_COLLECTION, task: task )
    end

    def yaml_targetdir_users( pathname_dir:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_targetdir( pathname_dir: pathname_dir, id: '', prefix: MetadataHelper2::PREFIX_USERS, task: task )
    end

    def yaml_targetdir_work( pathname_dir:, work:, task: MetadataHelper2::DEFAULT_TASK )
      yaml_targetdir( pathname_dir: pathname_dir, id: work.id, prefix: MetadataHelper2::PREFIX_WORK, task: task )
    end
    ...
    def self.init_attribute_names_always_include_cc
      rv = {}
      MetadataHelper2::ATTRIBUTE_NAMES_ALWAYS_INCLUDE_CC.each { |name| rv[name] = true }
      return rv
    end
