class GlobusCleanJob < GlobusJob
  ...
  def perform( concern_id, log_prefix: "Globus: ", clean_download: false )
    ...
    Rails.logger.debug "#{@globus_log_prefix} begin globus clean" unless @globus_job_quiet
    ...
    Rails.logger.debug "#{@globus_log_prefix} end globus clean" unless @globus_job_quiet
  end
  ...
  def globus_clean_job_email_rds( curation_concern: nil, description: '', log_provenance: false )
    ...
    email = WorkMailer.globus_clean_job_complete( to: EmailHelper.notification_email, body: msg ) ## TODO
    email.deliver_now
  end
